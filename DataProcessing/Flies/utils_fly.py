...
def extract_metrics_threshold_subsegment(foot_output, chunk, raw_data, tot_keep):
    """
    Extracts the contact metrics based on the threshold methods
    """
    ...
    local_keep_vector = tot_keep[local_begin:local_end]
    local_diff_keep = np.diff(np.float64(np.squeeze(local_keep_vector)))
    idx_begin_chunk = np.where(local_diff_keep==1)[0]
    if len(idx_begin_chunk)==0:
        return output_metrics
    else:
        for subsegment in range(len(idx_begin_chunk)):
            idx_end_chunk = np.where(local_diff_keep[idx_begin_chunk[subsegment]:]==-1)[0]
            if len(idx_end_chunk)==0:
                return output_metrics
            else:
                local_end_chunk = idx_end_chunk[0]
                local_sub_raw = local_matrix_raw[idx_begin_chunk[subsegment]:idx_begin_chunk[subsegment]+local_end_chunk,:,:]
                local_sub_foot = local_matrix_foot[idx_begin_chunk[subsegment]:idx_begin_chunk[subsegment]+local_end_chunk,:,:]
                time_vector = np.linspace(0,1/150*local_sub_foot.shape[0],local_sub_foot.shape[0])
                local_velocity = compute_velocity_com(np.squeeze(local_sub_raw[:,0,6]))
                for time in range(1,len(time_vector)-2):
                    if (local_sub_foot[time-1,0,0]==0 and local_sub_foot[time,0,0]==0 and local_sub_foot[time+1,0,0]!=0 and local_sub_foot[time+2,0,0]!=0):
                        init_pos_x = local_sub_foot[time+1,0,0]
                        init_pos_y = local_sub_foot[time+1,1,0]
                        idx_leg0 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,0]==1)[0]
                        idx_leg1 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,1]==1)[0]
                        idx_leg2 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,2]==1)[0]
                        idx_leg3 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,3]==1)[0]
                        idx_leg4 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,4]==1)[0]
                        idx_leg5 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,5]==1)[0]
                        if (len(idx_leg0)!=0):
                            final_pos_x = local_sub_foot[time+2+idx_leg0[0],0,0]
                            final_pos_y = local_sub_foot[time+2+idx_leg0[0],1,0]
                        ...
                        if (len(idx_leg1)!=0):
                            final_pos_x = local_sub_foot[time+2+idx_leg1[0],0,1]
                            final_pos_y = local_sub_foot[time+2+idx_leg1[0],1,1]
                        ...
                        if (len(idx_leg2)!=0):
                            final_pos_x = local_sub_foot[time+2+idx_leg2[0],0,2]
                            final_pos_y = local_sub_foot[time+2+idx_leg2[0],1,2]
                        ...
                        if (len(idx_leg3)!=0):
                            final_pos_x = local_sub_foot[time+2+idx_leg3[0],0,3]
                            final_pos_y = local_sub_foot[time+2+idx_leg3[0],1,3]
                        ...
                        if (len(idx_leg4)!=0):
                            final_pos_x = local_sub_foot[time+2+idx_leg4[0],0,4]
                            final_pos_y = local_sub_foot[time+2+idx_leg4[0],1,4]
                        ...
                        if (len(idx_leg5)!=0):
                            final_pos_x = local_sub_foot[time+2+idx_leg5[0],0,5]
                            final_pos_y = local_sub_foot[time+2+idx_leg5[0],1,5]
                    ...
                    # Second leg
                    elif (local_sub_foot[time-1,0,1]==0 and local_sub_foot[time,0,1]==0 and local_sub_foot[time+1,0,1]!=0 and local_sub_foot[time+2,0,1]!=0):
                        init_pos_x = local_sub_foot[time+1,0,1]
                        init_pos_y = local_sub_foot[time+1,1,1]
                        idx_leg0 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,0]==1)[0]
                        idx_leg1 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,1]==1)[0]
                        idx_leg2 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,2]==1)[0]
                        idx_leg3 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,3]==1)[0]
                    ...
                    # Third leg
                    elif (local_sub_foot[time-1,0,2]==0 and local_sub_foot[time,0,2]==0 and local_sub_foot[time+1,0,2]!=0 and local_sub_foot[time+2,0,2]!=0):
                        init_pos_x = local_sub_foot[time+1,0,2]
                        init_pos_y = local_sub_foot[time+1,1,2]
                        idx_leg0 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,0]==1)[0]
                        idx_leg1 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,1]==1)[0]
                        idx_leg2 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,2]==1)[0]
                        idx_leg3 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,3]==1)[0]
                    ...
                    # Fourth leg
                    elif (local_sub_foot[time-1,0,3]==0 and local_sub_foot[time,0,3]==0 and local_sub_foot[time+1,0,3]!=0 and local_sub_foot[time+2,0,3]!=0):
                        init_pos_x = local_sub_foot[time+1,0,3]
                        init_pos_y = local_sub_foot[time+1,1,3]
                        idx_leg0 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,0]==1)[0]
                        idx_leg1 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,1]==1)[0]
                        idx_leg2 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,2]==1)[0]
                        idx_leg3 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,3]==1)[0]
                    ...
                    # Fifth leg
                    elif (local_sub_foot[time-1,0,4]==0 and local_sub_foot[time,0,4]==0 and local_sub_foot[time+1,0,4]!=0 and local_sub_foot[time+2,0,4]!=0):
                        init_pos_x = local_sub_foot[time+1,0,4]
                        init_pos_y = local_sub_foot[time+1,1,4]
                        idx_leg0 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,0]==1)[0]
                        idx_leg1 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,1]==1)[0]
                        idx_leg2 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,2]==1)[0]
                        idx_leg3 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,3]==1)[0]
                    ...
                    # Sixth leg
                    elif (local_sub_foot[time-1,0,5]==0 and local_sub_foot[time,0,5]==0 and local_sub_foot[time+1,0,5]!=0 and local_sub_foot[time+2,0,5]!=0):
                        init_pos_x = local_sub_foot[time+1,0,5]
                        init_pos_y = local_sub_foot[time+1,1,5]
                        idx_leg0 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,0]==1)[0]
                        idx_leg1 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,1]==1)[0]
                        idx_leg2 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,2]==1)[0]
                        idx_leg3 = np.where(foot_contact_detection_threshold(local_sub_foot[time+2:,:,:6])[:,3]==1)[0]
...
def extract_metrics_threshold_touchdown(foot_output, chunk, raw_data):
    """Extracts the contact metrics based on the threshold method"""
    idx_nans = np.where(np.isnan(foot_output[:,0,0]))[0]
    output_metrics = np.zeros((1,8))
    local_begin = idx_nans[chunk]
    ...
    for time in range(1,len(time_vector)-2):
        # First leg
        if (local_matrix_foot[time-1,0,0]!=0 and local_matrix_foot[time,0,0]!=0 and local_matrix_foot[time+1,0,0]==0 and local_matrix_foot[time+2,0,0]==0):
            init_pos_x = local_matrix_foot[time,0,0]
            init_pos_y = local_matrix_foot[time,1,0]
            idx_leg0 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,0]==-1)[0]
            idx_leg1 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,1]==-1)[0]
            idx_leg2 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,2]==-1)[0]
            idx_leg3 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,3]==-1)[0]
            idx_leg4 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,4]==-1)[0]
            idx_leg5 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,5]==-1)[0]
            if (len(idx_leg0)!=0):
                final_pos_x = local_matrix_foot[time+idx_leg0[0],0,0]
                final_pos_y = local_matrix_foot[time+idx_leg0[0],1,0]
                output_metrics = np.vstack((output_metrics, np.array([chunk,1,1,1/150*(idx_leg0[0]+1),np.abs(final_pos_x-init_pos_x),np.abs(final_pos_y-init_pos_y),np.nanmean(local_velocity[time:time+idx_leg0[0]]),local_begin+time+1])))
            if (len(idx_leg1)!=0):
                final_pos_x = local_matrix_foot[time+idx_leg1[0],0,1]
                final_pos_y = local_matrix_foot[time+idx_leg1[0],1,1]
                output_metrics = np.vstack((output_metrics, np.array([chunk,1,2,1/150*(idx_leg1[0]+1),np.abs(final_pos_x-init_pos_x),np.abs(final_pos_y-init_pos_y),np.nanmean(local_velocity[time:time+idx_leg1[0]]),local_begin+time+1])))
            if (len(idx_leg2)!=0):
                final_pos_x = local_matrix_foot[time+idx_leg2[0],0,2]
                final_pos_y = local_matrix_foot[time+idx_leg2[0],1,2]
                output_metrics = np.vstack((output_metrics, np.array([chunk,1,3,1/150*(idx_leg2[0]+1),np.abs(final_pos_x-init_pos_x),np.abs(final_pos_y-init_pos_y),np.nanmean(local_velocity[time:time+idx_leg2[0]]),local_begin+time+1])))
            if (len(idx_leg3)!=0):
                final_pos_x = local_matrix_foot[time+idx_leg3[0],0,3]
                final_pos_y = local_matrix_foot[time+idx_leg3[0],1,3]
                output_metrics = np.vstack((output_metrics, np.array([chunk,1,4,1/150*(idx_leg3[0]+1),np.abs(final_pos_x-init_pos_x),np.abs(final_pos_y-init_pos_y),np.nanmean(local_velocity[time:time+idx_leg3[0]]),local_begin+time+1])))
            if (len(idx_leg4)!=0):
                final_pos_x = local_matrix_foot[time+idx_leg4[0],0,4]
                final_pos_y = local_matrix_foot[time+idx_leg4[0],1,4]
                output_metrics = np.vstack((output_metrics, np.array([chunk,1,5,1/150*(idx_leg4[0]+1),np.abs(final_pos_x-init_pos_x),np.abs(final_pos_y-init_pos_y),np.nanmean(local_velocity[time:time+idx_leg4[0]]),local_begin+time+1])))
            if (len(idx_leg5)!=0):
                final_pos_x = local_matrix_foot[time+idx_leg5[0],0,5]
                final_pos_y = local_matrix_foot[time+idx_leg5[0],1,5]
                output_metrics = np.vstack((output_metrics, np.array([chunk,1,6,1/150*(idx_leg5[0]+1),np.abs(final_pos_x-init_pos_x),np.abs(final_pos_y-init_pos_y),np.nanmean(local_velocity[time:time+idx_leg5[0]]),local_begin+time+1])))
        # Second leg
        if (local_matrix_foot[time-1,0,1]!=0 and local_matrix_foot[time,0,1]!=0 and local_matrix_foot[time+1,0,1]==0 and local_matrix_foot[time+2,0,1]==0):
            init_pos_x = local_matrix_foot[time,0,1]
            init_pos_y = local_matrix_foot[time,1,1]
            idx_leg0 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,0]==-1)[0]
            idx_leg1 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,1]==-1)[0]
            idx_leg2 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,2]==-1)[0]
            idx_leg3 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,3]==-1)[0]
            ...
            if (len(idx_leg5)!=0):
                ...
                output_metrics = np.vstack((output_metrics, np.array([chunk,2,6,1/150*(idx_leg5[0]+1),np.abs(final_pos_x-init_pos_x),np.abs(final_pos_y-init_pos_y),np.nanmean(local_velocity[time:time+idx_leg5[0]]),local_begin+time+1])))
        # Third leg
        if (local_matrix_foot[time-1,0,2]!=0 and local_matrix_foot[time,0,2]!=0 and local_matrix_foot[time+1,0,2]==0 and local_matrix_foot[time+2,0,2]==0):
            init_pos_x = local_matrix_foot[time,0,2]
            init_pos_y = local_matrix_foot[time,1,2]
            idx_leg0 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,0]==-1)[0]
            idx_leg1 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,1]==-1)[0]
            idx_leg2 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,2]==-1)[0]
            idx_leg3 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,3]==-1)[0]
            ...
            if (len(idx_leg5)!=0):
                ...
                output_metrics = np.vstack((output_metrics, np.array([chunk,3,6,1/150*(idx_leg5[0]+1),np.abs(final_pos_x-init_pos_x),np.abs(final_pos_y-init_pos_y),np.nanmedian(local_velocity[time:time+idx_leg5[0]]),local_begin+time+1])))
        # Fourth leg
        if (local_matrix_foot[time-1,0,3]!=0 and local_matrix_foot[time,0,3]!=0 and local_matrix_foot[time+1,0,3]==0 and local_matrix_foot[time+2,0,3]==0):
            init_pos_x = local_matrix_foot[time,0,3]
            init_pos_y = local_matrix_foot[time,1,3]
            idx_leg0 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,0]==-1)[0]
            idx_leg1 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,1]==-1)[0]
            idx_leg2 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,2]==-1)[0]
            idx_leg3 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,3]==-1)[0]
            ...
            if (len(idx_leg5)!=0):
                ...
                output_metrics = np.vstack((output_metrics, np.array([chunk,4,6,1/150*(idx_leg5[0]+1),np.abs(final_pos_x-init_pos_x),np.abs(final_pos_y-init_pos_y),np.nanmedian(local_velocity[time:time+idx_leg5[0]]),local_begin+time+1])))
        # Fifth leg
        if (local_matrix_foot[time-1,0,4]!=0 and local_matrix_foot[time,0,4]!=0 and local_matrix_foot[time+1,0,4]==0 and local_matrix_foot[time+2,0,4]==0):
            init_pos_x = local_matrix_foot[time,0,4]
            init_pos_y = local_matrix_foot[time,1,4]
            idx_leg0 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,0]==-1)[0]
            idx_leg1 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,1]==-1)[0]
            idx_leg2 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,2]==-1)[0]
            idx_leg3 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,3]==-1)[0]
            ...
            if (len(idx_leg5)!=0):
                ...
                output_metrics = np.vstack((output_metrics, np.array([chunk,5,6,1/150*(idx_leg5[0]+1),np.abs(final_pos_x-init_pos_x),np.abs(final_pos_y-init_pos_y),np.nanmedian(local_velocity[time:time+idx_leg5[0]]),local_begin+time+1])))
        # Sixth leg
        if (local_matrix_foot[time-1,0,5]!=0 and local_matrix_foot[time,0,5]!=0 and local_matrix_foot[time+1,0,5]==0 and local_matrix_foot[time+2,0,5]==0):
            init_pos_x = local_matrix_foot[time,0,5]
            init_pos_y = local_matrix_foot[time,1,5]
            idx_leg0 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,0]==-1)[0]
            idx_leg1 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,0]==-1)[0]
            idx_leg2 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,0]==-1)[0]
            idx_leg3 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,0]==-1)[0]
            idx_leg4 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,0]==-1)[0]
            idx_leg5 = np.where(foot_contact_detection_threshold(local_matrix_foot[time+1:,:,:6])[:,0]==-1)[0]
            if (len(idx_leg0)!=0):
                final_pos_x = local_matrix_foot[time+idx_leg0[0],0,0]
                final_pos_y = local_matrix_foot[time+idx_leg0[0],1,0]
                output_metrics = np.vstack((output_metrics, np.array([chunk,6,1,1/150*(idx_leg0[0]+1),np.abs(final_pos_x-init_pos_x),np.abs(final_pos_y-init_pos_y),np.nanmedian(local_velocity[time:time+idx_leg0[0]]),local_begin+time+1])))
...
def get_io_time_model_fr_stability(time_vector, line, foot_data, raw_data, marker, input_data, padding_time):
    """Extracts the gait cycle(s) preceding the contact (body prediction)"""
    # Compute the velocity forthe raw data
    raw_data_velocity = compute_local_velocity(raw_data)
    idx_meta = np.where(time_vector==line[-1])[0][0]
    idx_test = idx_meta - padding_time
    idx_next_fcmarker = np.where((np.abs(foot_data[idx_test+1:,0,marker-1]) - np.abs(foot_data[idx_test:-1,0,marker-1]))>0)[0]
    idx_next_fc1 = np.where((np.abs(foot_data[idx_test+1:,0,1]) - np.abs(foot_data[idx_test:-1,0,1]))>0)[0]
    idx_next_fc2 = np.where((np.abs(foot_data[idx_test+1:,0,3]) - np.abs(foot_data[idx_test:-1,0,3]))>0)[0]
    idx_next_fc3 = np.where((np.abs(foot_data[idx_test+1:,0,5]) - np.abs(foot_data[idx_test:-1,0,5]))>0)[0]
    idx_prev_fc1 = np.where((np.flip(np.abs(foot_data[:idx_test-1,0,1])) - np.flip(np.abs(foot_data[1:idx_test,0,1])))<0)[0]
    idx_prev_fc2 = np.where((np.flip(np.abs(foot_data[:idx_test-1,0,3])) - np.flip(np.abs(foot_data[1:idx_test,0,3])))<0)[0]
    idx_prev_fc3 = np.where((np.flip(np.abs(foot_data[:idx_test-1,0,5])) - np.flip(np.abs(foot_data[1:idx_test,0,5])))<0)[0]
    if ((len(idx_next_fc1)<2) | (len(idx_next_fc2)<2) | (len(idx_next_fc3)<2) | (len(idx_prev_fc1)==0) | (len(idx_prev_fc2)==0) | (len(idx_prev_fc3)==0)):
        return None, None
    ref_position_x_1, ref_position_y_1 = raw_data[idx_test,0,marker-1], raw_data[idx_test,1,marker-1]
    ref_position_x_2, ref_position_y_2 = raw_data[idx_test+idx_next_fcmarker[0]+1,0,marker-1], raw_data[idx_test+idx_next_fcmarker[0]+1,1,marker-1]
    time_input_1_1 = np.arange(idx_test-idx_prev_fc1[0], idx_test+idx_next_fc1[0]+1)
    time_input_2_1 = np.arange(idx_test-idx_prev_fc2[0], idx_test+idx_next_fc2[0]+1)
    time_input_3_1 = np.arange(idx_test-idx_prev_fc3[0], idx_test+idx_next_fc3[0]+1)
    time_input_1_2 = np.arange(idx_test+idx_next_fc1[0], idx_test+idx_next_fc1[1]+1)
    time_input_2_2 = np.arange(idx_test+idx_next_fc2[0], idx_test+idx_next_fc2[1]+1)
    time_input_3_2 = np.arange(idx_test+idx_next_fc3[0], idx_test+idx_next_fc3[1]+1)
    head_position_x_1_1, head_position_y_1_1 = raw_data[time_input_1_1,0,6] - ref_position_x_1, raw_data[time_input_1_1,1,6] - ref_position_y_1
    head_position_x_2_1, head_position_y_2_1 = raw_data[time_input_2_1,0,6] - ref_position_x_1, raw_data[time_input_2_1,1,6] - ref_position_y_1
    head_position_x_3_1, head_position_y_3_1 = raw_data[time_input_3_1,0,6] - ref_position_x_1, raw_data[time_input_3_1,1,6] - ref_position_y_1
    head_velocity_x_1_1, head_velocity_y_1_1 = raw_data_velocity[time_input_1_1,0,6], raw_data_velocity[time_input_1_1,1,6]
    head_velocity_x_2_1, head_velocity_y_2_1 = raw_data_velocity[time_input_2_1,0,6], raw_data_velocity[time_input_2_1,1,6]
    head_velocity_x_3_1, head_velocity_y_3_1 = raw_data_velocity[time_input_3_1,0,6], raw_data_velocity[time_input_3_1,1,6]

    head_position_x_1_2, head_position_y_1_2 = raw_data[time_input_1_2,0,6] - ref_position_x_2, raw_data[time_input_1_2,1,6] - ref_position_y_2
    head_position_x_2_2, head_position_y_2_2 = raw_data[time_input_2_2,0,6] - ref_position_x_2, raw_data[time_input_2_2,1,6] - ref_position_y_2
    head_position_x_3_2, head_position_y_3_2 = raw_data[time_input_3_2,0,6] - ref_position_x_2, raw_data[time_input_3_2,1,6] - ref_position_y_2
    head_velocity_x_1_2, head_velocity_y_1_2 = raw_data_velocity[time_input_1_2,0,6], raw_data_velocity[time_input_1_2,1,6]
    head_velocity_x_2_2, head_velocity_y_2_2 = raw_data_velocity[time_input_2_2,0,6], raw_data_velocity[time_input_2_2,1,6]
    head_velocity_x_3_2, head_velocity_y_3_2 = raw_data_velocity[time_input_3_2,0,6], raw_data_velocity[time_input_3_2,1,6]
    # Interpolation of the inputs
    output_time = np.linspace(0,1,21)
    head_position_x_1_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_position_x_1_1)), head_position_x_1_1),-1)
    head_position_x_2_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_position_x_2_1)), head_position_x_2_1),-1)
    head_position_x_3_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_position_x_3_1)), head_position_x_3_1),-1)
    head_velocity_x_1_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_x_1_1)), head_velocity_x_1_1),-1)
    head_velocity_x_2_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_x_2_1)), head_velocity_x_2_1),-1)
    head_velocity_x_3_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_x_3_1)), head_velocity_x_3_1),-1)
    head_position_y_1_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_position_y_1_1)), head_position_y_1_1),-1)
    head_position_y_2_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_position_y_2_1)), head_position_y_2_1),-1)
    head_position_y_3_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_position_y_3_1)), head_position_y_3_1),-1)
    head_velocity_y_1_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_y_1_1)), head_velocity_y_1_1),-1)
    head_velocity_y_2_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_y_2_1)), head_velocity_y_2_1),-1)
    head_velocity_y_3_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_y_3_1)), head_velocity_y_3_1),-1)

    head_position_x_1_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_position_x_1_2)), head_position_x_1_2),-1)
    head_position_x_2_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_position_x_2_2)), head_position_x_2_2),-1)
    head_position_x_3_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_position_x_3_2)), head_position_x_3_2),-1)
    head_velocity_x_1_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_x_1_2)), head_velocity_x_1_2),-1)
    head_velocity_x_2_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_x_2_2)), head_velocity_x_2_2),-1)
    head_velocity_x_3_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_x_3_2)), head_velocity_x_3_2),-1)
    head_position_y_1_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_position_y_1_2)), head_position_y_1_2),-1)
    head_position_y_2_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_position_y_2_2)), head_position_y_2_2),-1)
    head_position_y_3_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_position_y_3_2)), head_position_y_3_2),-1)
    head_velocity_y_1_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_y_1_2)), head_velocity_y_1_2),-1)
    head_velocity_y_2_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_y_2_2)), head_velocity_y_2_2),-1)
    head_velocity_y_3_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_y_3_2)), head_velocity_y_3_2),-1)
    head_position_x_1_ = np.concatenate((head_position_x_1_1_, head_position_x_1_2_),0)
    head_position_y_1_ = np.concatenate((head_position_y_1_1_, head_position_y_1_2_),0)
    head_velocity_x_1_ = np.concatenate((head_velocity_x_1_1_, head_velocity_x_1_2_),0)
    head_velocity_y_1_ = np.concatenate((head_velocity_y_1_1_, head_velocity_y_1_2_),0)

    head_position_x_2_ = np.concatenate((head_position_x_2_1_, head_position_x_2_2_),0)
    head_position_y_2_ = np.concatenate((head_position_y_2_1_, head_position_y_2_2_),0)
    head_velocity_x_2_ = np.concatenate((head_velocity_x_2_1_, head_velocity_x_2_2_),0)
    head_velocity_y_2_ = np.concatenate((head_velocity_y_2_1_, head_velocity_y_2_2_),0)

    head_position_x_3_ = np.concatenate((head_position_x_3_1_, head_position_x_3_2_),0)
    head_position_y_3_ = np.concatenate((head_position_y_3_1_, head_position_y_3_2_),0)
    head_velocity_x_3_ = np.concatenate((head_velocity_x_3_1_, head_velocity_x_3_2_),0)
    head_velocity_y_3_ = np.concatenate((head_velocity_y_3_1_, head_velocity_y_3_2_),0)
    input_array = np.hstack((head_position_x_1_, head_position_y_1_, head_velocity_x_1_, head_velocity_y_1_, head_position_x_2_, head_position_y_2_,
                             head_velocity_x_2_, head_velocity_y_2_, head_position_x_3_, head_position_y_3_, head_velocity_x_3_, head_velocity_y_3_))
    time_cycle = line[3]
    ...
    if len(idx_same2)==0:
        return input_array, None
    final_position_x_2 = raw_data[idx_test+idx_next_fc1[0]+2,0,1] - ref_position_x_1
    final_position_y_2 = raw_data[idx_test+idx_next_fc1[0]+2,1,1] - ref_position_y_1
    time_contact_2 = (input_data[idx_same2,3]/time_cycle)[0]

    idx_same3 = np.where((input_data[:,0]==line[0]) & (input_data[:,-1]==line[-1]) & (input_data[:,2]==4))[0]
    if len(idx_same3)==0:
        return input_array, None
    final_position_x_3 = raw_data[idx_test+idx_next_fc2[0]+2,0,3] - ref_position_x_1
    final_position_y_3 = raw_data[idx_test+idx_next_fc2[0]+2,1,3] - ref_position_y_1
    time_contact_3 = (input_data[idx_same3,3]/time_cycle)[0]

    idx_same4 = np.where((input_data[:,0]==line[0]) & (input_data[:,-1]==line[-1]) & (input_data[:,2]==6))[0]
    if len(idx_same4)==0:
        return input_array, None
    final_position_x_4 = raw_data[idx_test+idx_next_fc3[0]+2,0,5] - ref_position_x_1
    final_position_y_4 = raw_data[idx_test+idx_next_fc3[0]+2,1,5] - ref_position_y_1
    time_contact_4 = (input_data[idx_same4,3]/time_cycle)[0]
...
def get_io_time_model_fr_phase(time_vector, line, foot_data, raw_data, marker, input_data, padding_time, input_phase, input_modulus):
    """Extracts the gait cycle preceding the contact (body prediction)"""
    # Compute the velocity for the raw data 
    raw_data_velocity = compute_local_velocity(raw_data)
    idx_meta = np.where(time_vector==line[-1])[0][0]
    idx_test = idx_meta - padding_time
    if idx_test > foot_data.shape[0]:
    ...
    time_input_2 = np.arange(idx_test-idx_prev_fc2[0], idx_test+idx_next_fc2[0]+1)
    time_input_3 = np.arange(idx_test-idx_prev_fc3[0], idx_test+idx_next_fc3[0]+1)
    # Computing the local phase and modulus
    local_phase_1, local_phase_2, local_phase_3 = input_phase[time_input_1,:], input_phase[time_input_2,:], input_phase[time_input_3,:] 
    local_modulus_1, local_modulus_2, local_modulus_3 = input_modulus[time_input_1,:], input_modulus[time_input_2,:], input_modulus[time_input_3,:] 
    ...
    head_velocity_y_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_y_2)), head_velocity_y_2),-1)
    head_velocity_y_3_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(head_velocity_y_3)), head_velocity_y_3),-1)
    local_phase_ = np.zeros((21,6,3))
    local_modulus_ = np.zeros((6,3))
    for subline in range(6):
        local_phase_[:,subline,0] = np.interp(output_time, np.linspace(0,1,local_phase_1.shape[0]), local_phase_1[:,subline])
        local_phase_[:,subline,1] = np.interp(output_time, np.linspace(0,1,local_phase_2.shape[0]), local_phase_2[:,subline])
        local_phase_[:,subline,2] = np.interp(output_time, np.linspace(0,1,local_phase_3.shape[0]), local_phase_3[:,subline])
        local_modulus_[subline,0] = np.nanmean(local_modulus_1[:,subline])
        local_modulus_[subline,1] = np.nanmean(local_modulus_2[:,subline])
        local_modulus_[subline,2] = np.nanmean(local_modulus_3[:,subline])
    ...
    idx_same2 = np.where((input_data[:,0]==line[0]) & (input_data[:,-1]==line[-1]) & (input_data[:,2]==2))[0]
    if len(idx_same2)==0:
        return input_array, None, None, None
    final_position_x_2 = raw_data[idx_test+idx_next_fc1[0]+2,0,1] - ref_position_x
    final_position_y_2 = raw_data[idx_test+idx_next_fc1[0]+2,1,1] - ref_position_y
    time_contact_2 = (input_data[idx_same2,3]/time_cycle)[0]
    ...
    output_array = np.expand_dims(np.array([final_position_x_2, final_position_y_2, time_contact_2, final_position_x_3, final_position_y_3, time_contact_3, final_position_x_4, final_position_y_4, time_contact_4]),-1)
    return input_array, output_array, local_phase_, local_modulus_
...
def get_io_time_model_self_fr_touchdown(time_vector, line, foot_data, raw_data, marker, input_data, padding_time):
    """Extracts the gait cycle preceding the contact (self prediction)"""
    # Compute the velocity for the raw data 
    raw_data_velocity = compute_local_velocity(raw_data)
    idx_meta = np.where(time_vector==line[-1])[0][0]
    ...
    idx_prev_fc2 = np.where((np.flip(np.abs(foot_data[:idx_test-1,0,3])) - np.flip(np.abs(foot_data[1:idx_test,0,3])))>0)[0]
    idx_prev_fc3 = np.where((np.flip(np.abs(foot_data[:idx_test-1,0,5])) - np.flip(np.abs(foot_data[1:idx_test,0,5])))>0)[0]
    if ((len(idx_next_fc1)==0) | (len(idx_next_fc2)==0) | (len(idx_next_fc3)==0) | (len(idx_prev_fc1)<2) | (len(idx_prev_fc2)<2) | (len(idx_prev_fc3)<2)):
        return None, None
    ref_position_x, ref_position_y = foot_data[idx_test-1,0,marker-1], foot_data[idx_test-1,1,marker-1]
    time_input_1 = np.arange(idx_test-idx_prev_fc1[1], idx_test+idx_next_fc1[0]+1)
    time_input_2 = np.arange(idx_test-idx_prev_fc2[1], idx_test+idx_next_fc2[0]+1)
    time_input_3 = np.arange(idx_test-idx_prev_fc3[1], idx_test+idx_next_fc3[0]+1)
    leg_position_x_1, leg_position_y_1 = raw_data[time_input_1,0,1] - ref_position_x, raw_data[time_input_1,1,1] - ref_position_y
    leg_position_x_2, leg_position_y_2 = raw_data[time_input_2,0,3] - ref_position_x, raw_data[time_input_2,1,3] - ref_position_y
    leg_position_x_3, leg_position_y_3 = raw_data[time_input_3,0,5] - ref_position_x, raw_data[time_input_3,1,5] - ref_position_y
    leg_velocity_x_1, leg_velocity_y_1 = raw_data_velocity[time_input_1,0,1], raw_data_velocity[time_input_1,1,1]
    ...
    # Interpolation of the inputs 
    output_time = np.linspace(0,1,41)
    leg_position_x_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(leg_position_x_1)),leg_position_x_1),-1)
    leg_position_x_2_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(leg_position_x_2)),leg_position_x_2),-1)
    leg_position_x_3_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(leg_position_x_3)),leg_position_x_3),-1)
    leg_position_y_1_ = np.expand_dims(np.interp(output_time, np.linspace(0,1,len(leg_position_y_1)),leg_position_y_1),-1)
...
def get_io_time_model_fr_abs_touchdown(time_vector, line, foot_data, raw_data, marker, input_data, padding_time):
    """Extracts the gait cycle preceding the contact (body prediction)"""
    # Compute the velocity for the raw data 
    raw_data_velocity = compute_local_velocity(raw_data)
    idx_meta = np.where(time_vector==line[-1])[0][0]
    ...
    if ((len(idx_next_fc1)==0) | (len(idx_next_fc2)==0) | (len(idx_next_fc3)==0) | (len(idx_prev_fc1)<2) | (len(idx_prev_fc2)<2) | (len(idx_prev_fc3)<2)):
        return None, None
    ref_position_x, ref_position_y = foot_data[idx_test-1,0,marker-1], foot_data[idx_test-1,1,marker-1]
    print(ref_position_x,ref_position_y)
    time_input_1 = np.arange(idx_test-idx_prev_fc1[1], idx_test+idx_next_fc1[0]+1)
    time_input_2 = np.arange(idx_test-idx_prev_fc2[1], idx_test+idx_next_fc2[0]+1)
    time_input_3 = np.arange(idx_test-idx_prev_fc3[1], idx_test+idx_next_fc3[0]+1)
    head_position_x_1, head_position_y_1 = raw_data[time_input_1,0,6] - ref_position_x, raw_data[time_input_1,1,6] - ref_position_y
    ...
    final_position_x_2 = raw_data[idx_test+idx_next_fc1[0]+2,0,1] - ref_position_x
    final_position_y_2 = raw_data[idx_test+idx_next_fc1[0]+2,1,1] - ref_position_y
    time_contact_2 = (input_data[idx_same2,3])[0]
    ...
    final_position_x_3 = raw_data[idx_test+idx_next_fc2[0]+2,0,3] - ref_position_x
    final_position_y_3 = raw_data[idx_test+idx_next_fc2[0]+2,1,3] - ref_position_y
    time_contact_3 = (input_data[idx_same3,3])[0]
    ...
    final_position_x_4 = raw_data[idx_test+idx_next_fc3[0]+2,0,5] - ref_position_x
    final_position_y_4 = raw_data[idx_test+idx_next_fc3[0]+2,1,5] - ref_position_y
    time_contact_4 = (input_data[idx_same4,3])[0]

    output_array = np.expand_dims(np.array([final_position_x_2, final_position_y_2, time_contact_2, final_position_x_3, final_position_y_3, time_contact_3, final_position_x_4, final_position_y_4, time_contact_4]),-1)
    return input_array, output_array


def get_io_time_model_fr_abs(time_vector, line, foot_data, raw_data, marker, input_data, padding_time):
    """Extracts the gait cycle preceding the contact (body prediction)"""
    # Compute the velocity for the raw data 
    raw_data_velocity = compute_local_velocity(raw_data)
    idx_meta = np.where(time_vector==line[-1])[0][0]
...
def get_io_time_model_fr_or(time_vector, line, foot_data, raw_data, orientation_data, marker, input_data, padding_time):
    """Extracts the gait cycle preceding the contact (body prediction)"""
    # Compute the velocity for the raw data 
    raw_data_velocity = compute_local_velocity(raw_data)
    idx_meta = np.where(time_vector==line[-1])[0][0]
    ...
    head_velocity_x_2, head_velocity_y_2 = raw_data_velocity[time_input_2,0,6], raw_data_velocity[time_input_2,1,6]
    head_velocity_x_3, head_velocity_y_3 = raw_data_velocity[time_input_3,0,6], raw_data_velocity[time_input_3,1,6]
    orientation_1 = orientation_data[time_input_1]
    orientation_2 = orientation_data[time_input_2]
    orientation_3 = orientation_data[time_input_3]
...
def get_io_time_model_fl_or(time_vector, line, foot_data, raw_data, orientation_data, marker, input_data, padding_time):
    """Extracts the gait cycle preceding the contact (body prediction)"""
    # Compute the velocity for the raw data 
    raw_data_velocity = compute_local_velocity(raw_data)
    idx_meta = np.where(time_vector==line[-1])[0][0]
...
def get_video_id_chunk_(padded_video, validity):
    """Gets the video id of each valid chunks"""
    idx_nans = np.where(np.isnan(padded_video))[0]
    n_chunks = len(idx_nans) - 1
    output_list = []
...
def get_input_output_fr_stability(padded_raw, padded_foot, tot_list_data, validity):
    """
    Gets the inputs and outputs for the stability analyses
    """
    ...
    for chunk in tqdm(range(n_chunk)):
        if validity[chunk]!=4:
            ...
            total_input = np.zeros((len(idx_to_keep),42,12))
            total_output = np.zeros((len(idx_to_keep),9))
            for line in range(len(idx_to_keep)):
                tmp_input, tmp_output = get_io_time_model_fr_stability(time_vector, input_data[idx_to_keep[line],:], padded_foot[local_idx,:,:], padded_raw[local_idx,:,:], 1, input_data, idx_nans[chunk]+1)
...
def get_input_output_fr_or(padded_raw, padded_foot, tot_list_data, validity, orientation_data):
    ...
    for chunk in tqdm(range(n_chunk)):
        if validity[chunk]!=4:
            ...
            total_output = np.zeros((len(idx_to_keep),9))
            for line in range(len(idx_to_keep)):
                tmp_input, tmp_output = get_io_time_model_fr_or(time_vector, input_data[idx_to_keep[line],:], padded_foot[local_idx,:,:], padded_raw[local_idx,:,:], orientation_data[local_idx], 1, input_data, idx_nans[chunk]+1)
                if tmp_output is None:
                    total_input[line,:], total_output[line,:] = np.nan, np.nan
                elif tmp_output.shape[1]!=0:
                    total_input[line,:] = tmp_input
...
def get_input_output_fr_(padded_raw, padded_foot, tot_list_data, validity):
    """Gets the input and outputs for the controller inference for the first tripod gait - body prediction - for the subsegments"""
    idx_nans = np.where(np.isnan(padded_raw[:,0,0]))[0]
    n_chunk = len(idx_nans)-1
    new_input, new_output = [], []
    time_vector = np.arange(0,padded_raw.shape[0])
    count_chunk = 0
    print(len(validity),n_chunk)
    for chunk in tqdm(range(n_chunk)):
        if validity[chunk]==4:
            continue
        else:
...
def get_input_output_fl_(padded_raw, padded_foot, tot_list_data, validity):
...
def get_input_output_self_fr_touchdown(padded_raw, padded_foot, tot_list_data, validity):
    ...
    for chunk in tqdm(range(n_chunk)):
        if validity[chunk]!=4:
            ...
            local_idx = np.arange(idx_nans[chunk]+1,idx_nans[chunk+1])
            idx_to_keep = np.where((input_data[:,1]==1) & (input_data[:,2]==1) & (input_data[:,3]>0.075))[0]
            total_input = np.zeros((len(idx_to_keep),41,12))
            total_output = np.zeros((len(idx_to_keep),9))
            for line in range(len(idx_to_keep)):
                tmp_input, tmp_output = get_io_time_model_self_fr_touchdown(time_vector, input_data[idx_to_keep[line],:], padded_foot[local_idx,:,:], padded_raw[local_idx,:,:], 1, input_data, idx_nans[chunk]+1)
                if tmp_output is None:
                    total_input[line,:], total_output[line,:] = np.nan, np.nan
                elif tmp_output.shape[1]!=0:
                    total_input[line,:] = tmp_input
...
def get_input_output_fr_vel_abs_touchdown(padded_raw, padded_foot, tot_list_data, validity):
    """Gets the input and outputs for the controller inferene for the first tripod gait - body prediction with saving the average velocity over the bout"""
    idx_nans = np.where(np.isnan(padded_raw[:,0,0]))[0]
    n_chunk = len(idx_nans)-1
    new_input, new_output = [], []
    vel_avg = []
    ...
    for chunk in tqdm(range(n_chunk)):
        if validity[chunk]!=4:
            ...
            total_input = np.zeros((len(idx_to_keep),41,12))
            total_output = np.zeros((len(idx_to_keep),9))
            local_head_data = padded_raw[local_idx,0,6]
            local_head_vel = np.zeros((local_head_data.shape[0],))
            dt = 1/150
            local_head_vel[2:-2] = (- local_head_data[4:] + 8 * local_head_data[3:-1] - 8 * local_head_data[1:-3] + local_head_data[:-4]) / (12*dt)
            #avg_velocity_bout = (local_matrix_data[-1,0,6] - local_matrix_data[0,0,6]) / (1/150 * len(local_idx))
            avg_velocity_bout = np.nanmean(local_head_vel[2:-2])
            vel_avg.append(avg_velocity_bout)
            for line in range(len(idx_to_keep)):
                tmp_input, tmp_output = get_io_time_model_fr_abs_touchdown(time_vector, input_data[idx_to_keep[line],:], padded_foot[local_idx,:,:], padded_raw[local_idx,:,:], 1, input_data, idx_nans[chunk]+1)
                if tmp_output is None:
                    total_input[line,:], total_output[line,:] = np.nan, np.nan
                elif tmp_output.shape[1]!=0:
                    total_input[line,:] = tmp_input
            ...
            new_input.append(total_input)
            new_output.append(total_output)
    return new_input, new_output, vel_avg


def get_input_output_fr_vel_abs(padded_raw, padded_foot, tot_list_data, validity):
    """Gets the input and outputs for the controller inferene for the first tripod gait - body prediction with saving the average velocity over the bout"""
    idx_nans = np.where(np.isnan(padded_raw[:,0,0]))[0]
    n_chunk = len(idx_nans)-1
    new_input, new_output = [], []
    ...
    for chunk in tqdm(range(n_chunk)):
        if validity[chunk]!=4:
            ...
            vel_avg.append(avg_velocity_bout)
            for line in range(len(idx_to_keep)):
                tmp_input, tmp_output = get_io_time_model_fr_abs(time_vector, input_data[idx_to_keep[line],:], padded_foot[local_idx,:,:], padded_raw[local_idx,:,:], 1, input_data, idx_nans[chunk]+1)
                if tmp_output is None:
                    total_input[line,:], total_output[line,:] = np.nan, np.nan
                elif tmp_output.shape[1]!=0:
                    total_input[line,:] = tmp_input
...
def get_input_output_fr_vel(padded_raw, padded_foot, tot_list_data, validity):
    """Gets the input and outputs for the controller inferene for the first tripod gait - body prediction with saving the average velocity over the bout"""
    idx_nans = np.where(np.isnan(padded_raw[:,0,0]))[0]
    n_chunk = len(idx_nans)-1
    new_input, new_output = [], []
...
def get_input_output_fr_phase(padded_raw, padded_foot, tot_list_data, validity):
    ...
    for chunk in tqdm(range(n_chunk)):
        if validity[chunk]!=4:
            ...
            total_input = np.zeros((len(idx_to_keep),21,12))
            total_output = np.zeros((len(idx_to_keep),9))
            total_phase = np.zeros((len(idx_to_keep),21,6,3))
            total_modulus = np.zeros((len(idx_to_keep),6,3))
            # Compute differences for phase calculation
            local_leg_difference = np.zeros((len(local_idx),6))
            local_leg_phase, local_leg_modulus = np.zeros((len(local_idx),6)), np.zeros((len(local_idx),6))
            for leg in range(6):
                local_leg_difference[:,leg] = np.squeeze(padded_raw[local_idx,0,leg] - padded_raw[local_idx,0,6])
                local_std = np.nanstd(local_leg_difference[:,leg])
                local_leg_difference[:,leg] = (local_leg_difference[:,leg] - np.nanmean(local_leg_difference[:,leg]))/local_std
                local_leg_phase[:,leg] = savgol_filter(np.unwrap(np.angle(hilbert(np.squeeze(local_leg_difference[:,leg])))),5,1)
                local_leg_modulus[:,leg] = np.abs(hilbert(np.squeeze(local_leg_difference[:,leg])))
            
            for line in range(len(idx_to_keep)):
                tmp_input, tmp_output, tmp_phase, tmp_modulus = get_io_time_model_fr_phase(time_vector, input_data[idx_to_keep[line],:], padded_foot[local_idx,:,:], padded_raw[local_idx,:,:], 1, input_data, idx_nans[chunk]+1, local_leg_phase, local_leg_modulus)
                if tmp_output is None:
                    total_input[line,:], total_output[line,:] = np.nan, np.nan
                    total_phase[line,:], total_modulus[line,:] = np.nan, np.nan
                elif tmp_output.shape[1]!=0:
                    total_input[line,:] = tmp_input
                    total_output[line,0] = tmp_output[0][0]
                    total_output[line,1] = tmp_output[1][0]
                    ...
                    total_output[line,7] = tmp_output[7][0]
                    total_output[line,8] = tmp_output[8][0]
                    total_phase[line,:,:,:] = tmp_phase
                    total_modulus[line,:,:] = tmp_modulus
                else:
                    total_input[line,:], total_output[line,:] = np.nan, np.nan
                    total_phase[line,:], total_modulus[line,:] = np.nan, np.nan
            count_chunk += 1
            new_input.append(total_input)
            new_output.append(total_output)
            new_phase.append(total_phase)
            new_modulus.append(total_modulus)
    return new_input, new_output, new_phase, new_modulus
...
def get_input_output_fl_or(padded_raw, padded_foot, tot_list_data, validity, orientation_data):
    """Gets the input and outputs for the controller inference for the first tripod gait"""
    idx_nans = np.where(np.isnan(padded_raw[:,0,0]))[0]
    n_chunk = len(idx_nans)-1
    new_input, new_output = [], []
    ...
    for chunk in tqdm(range(n_chunk)):
        if validity[chunk]!=4:
            ...
            total_output = np.zeros((len(idx_to_keep),9))
            for line in range(len(idx_to_keep)):
                tmp_input, tmp_output = get_io_time_model_fl_or(time_vector, input_data[idx_to_keep[line],:], padded_foot[local_idx,:,:], padded_raw[local_idx,:,:], orientation_data[local_idx], 4, input_data, idx_nans[chunk]+1)
                if tmp_output is None:
                    total_input[line,:], total_output[line,:] = np.nan, np.nan
                elif tmp_output.shape[1]!=0:
                    total_input[line,:] = tmp_input
...
def get_input_output_self_fr_(padded_raw, padded_foot, tot_list_data, validity):
    """Gets the input and outpus for the controller inference for the first tripod gait - self prediction"""
    idx_nans = np.where(np.isnan(padded_raw[:,0,0]))[0]
    n_chunk = len(idx_nans)-1
    new_input, new_output = [], []
...
def get_input_output_self_fl_(padded_raw, padded_foot, tot_list_data, validity):
...
def regression_fr_body_or(velocity_tier, tot_input, tot_output, tot_video, id_leg):
    """Computes the regression coefficient for the fly data"""
    n_videos = len(set(tot_video[~np.isnan(tot_video)]))
    regression_matrix = np.zeros((n_videos,21,5,3,5,5))
    for video in tqdm(range(n_videos)):
        for vel_tier in range(5):
            local_idx = np.where((tot_video[:,0]==video+1) & (velocity_tier==vel_tier+1))[0]
            idx_nan = np.where(~np.isnan(tot_input[local_idx,0,0]))[0]
    ...
    for video in range(n_videos):
        ...
        for time in range(tot_input.shape[1]):
            ...
            if design_mat.shape[0]<10:
                rsquare_diagonal[video, time] = np.nan
            else:
                rsquare_diagonal[video, time] = multilinear_ols_rsquare(design_mat, pred_value)
